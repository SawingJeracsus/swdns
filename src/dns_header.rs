...
impl DnsHeader {
    pub fn read(&mut self, buffer: &mut BytePacketBuffer) -> BytePacketBufferResult<()> {
        ...
        let b = (flags & 0xFF) as u8; // next 8

        // a & (1 << n) > 0 - return true if n bit from the end is 1
    ...
    pub fn write(&self, buffer: &mut BytePacketBuffer) -> BytePacketBufferResult<()> {
        ...
        /*
                Builds next binary
                |response 1bit| |opcode 4bits| |authoritatice_answer 1bit| |truncated_message 1bit| |recursion_desired 1bit
        */
        buffer.write_u8(
